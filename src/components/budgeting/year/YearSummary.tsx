...
import { MONTHS_LONG, MONTHS_SHORT } from './constants'

export function YearSummary({ annual, completed, projected, currentLabel, effectiveStartMonth, year }: {
  annual: YearAnnual
  completed: number
  projected: number
  ...
  const rangeLabel = effectiveStartMonth != null
    ? `${MONTHS_SHORT[effectiveStartMonth]} – Dec ${year}`
    : 'full year'

  return (
    <SurfaceCard>
      <Row align="stretch" sx={{ py: 2.5, px: 1 }}>
        <Stat label="Income" value={fmtMoney(annual.income)} sub={`${fmtMoney(annual.monthlyIncome)}/mo · ${rangeLabel}`} sx={cellSx()} />
        <VDivider sx={{ my: 0.75 }} />
