...
- **Theme** — `src/lib/theme.ts` (`'use client'`) holds the single `createTheme`: the "Anchor" brand (teal `primary`, Switzer typography, soft radii, `MuiButton`/`MuiPaper`/`MuiChip`/`MuiLinearProgress` overrides). It also exports a `brand` object of raw scales (anchor/zinc, `cardGradient`, `shadow`, `accentSoft`) for the few spots that need literal values — gradient `CardTile`s, the teal-soft headline panel, `StatusChip` colors. Reach for theme tokens (`sx`, `palette.*`) first; use `brand` only when no token fits.
